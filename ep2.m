...
function retval = avaliav (tipo, x, y, v, nx, ny, ax, bx, ay, by)
...
v = constroiv('bilinear', nx, ny, ax, bx, ay, by, f(1:m:end,1:m:end));
...
for i = 1:nmax
	for j = 1:nmax
		xi = ax + (i-1) * hx;
		yj = ay + (j-1) * hy;
		s(i,j) = avaliav('bilinear', xi, yj, v, nx, ny, ax, bx, ay, by);
